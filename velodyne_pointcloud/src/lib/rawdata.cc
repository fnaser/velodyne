...
namespace velodyne_rawdata
{
                       ...
                       VPointCloud &pc)
  {
    ...
    for (int i = 0; i < BLOCKS_PER_PACKET; i++) {
      ...
      uint8_t bank_origin = 0;
      if (raw->blocks[i].header == LOWER_BANK) {
        // lower bank lasers are [32..63]
        bank_origin = 32;
      }

      for (uint8_t j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
        VPoint point;
        uint8_t laser_number = j + bank_origin;
        unpack_one_point(i,k,laser_number, raw, point);
  ...
  /** @brief convert raw packet to two pointclouds, overloaded for dual return mode
   *
   *  @param pkt raw packet to unpack
   *  @param pc_strong shared pointer to point cloud for strongest returns (points are appended)
   *  @param pc_last shared pointer to point cloud for last returns (points are appended)
   */
  void RawData::unpack(const velodyne_msgs::VelodynePacket &pkt, VPointCloud &pc_strong, VPointCloud &pc_last)
  {
    ROS_DEBUG_STREAM("Received packet, time: " << pkt.stamp);
    ...
    for (int i = 0; i < BLOCKS_PER_PACKET; i++) {
      ...
      for (uint8_t j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
        ...
        // append this point to the appropriate pointcloud
        if((int)std::floor(i/2) % 2 == 0) {
          // Strong return
          pc_strong.push_back(point);
        }else {
          pc_last.push_back(point);
        }
      }
    }
  }

  /** @brief convert one point from a raw packet to a VPoint
 *
 *  @param i block_id
 *  @param k point_id
 *  @param laser_number the laser_id
 *  @param raw a pointer to the raw packet data
 *  @param point a reference to a VPoint in which to store the computed point
 */
  void RawData::unpack_one_point(int i, int k, uint8_t laser_number, const raw_packet_t *raw, VPoint &point) {
